        ...
        students.forEach((s,i)=>{
          t.innerHTML += `
            <tr>
              ...
              <td>${s.phone||''}</td>
              <td>${s.email||''}</td>
              <td style="text-align: center;">${s.requirements_completed==1 ? `<a href="view_assessment.php?student_id=${encodeURIComponent(s.id)}" target="_blank" title="View Physical Assessment"><i class="fas fa-file-medical"></i></a>` : 'N/A'}</td>
              <td>${s.emergency_contact||''}</td>
              <td style="text-align: center;"><a href="appointment_history.php?student_id=${encodeURIComponent(s.id)}" target="_blank" title="View Appointment History"><i class="fas fa-eye"></i></a></td>
            </tr>`;
        });
      }
    })
