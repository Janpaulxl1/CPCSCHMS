...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['visit_id'])) {
    ...
    if ($update_stmt->execute()) {
        ...
        if ($visit) {
            $student_id_val = $visit['student_id_string'] ?? null;
            $student_name_val = $visit['student_name'] ?? 'Walk-in';

            // Insert into appointments table conditionally
            if ($student_id_val) {
                ...
                $insert_appointment_stmt->bind_param("sss", $student_id_val, $visit['visit_date'], $visit['reason']);
            } else {
                $insert_appointment_sql = "INSERT INTO appointments (appointment_time, reason, status) VALUES (?, ?, 'Completed')";
                $insert_appointment_stmt = $conn->prepare($insert_appointment_sql);
                $insert_appointment_stmt->bind_param("ss", $visit['visit_date'], $visit['reason']);
            }
            if ($insert_appointment_stmt->execute()) {
                ...
                $insert_log_stmt->bind_param("issss", $appointment_id, $date, $time, $visit['reason'], $student_name_val);
